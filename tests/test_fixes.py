...
def _mk_run(base, eval_id, cfg, run, pr, metrics=None, timing=None):
    ...
    passed = round(pr * 2)
    (d / "grading.json").write_text(json.dumps({"summary": {"pass_rate": pr, "passed": passed, "failed": 2 - passed, "total": 2}}))
...
def test_load_runs_optional_metrics_timing(tmp_path):
    _mk_run(tmp_path, 0, "with_skill", 1, 1.0,
...
def test_benchmark_export_flows_metrics_and_n(tmp_path):
    runs = tmp_path / "runs"
    _mk_run(runs, 0, "with_skill", 1, 1.0,
